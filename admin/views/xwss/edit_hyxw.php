        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <?php $form = ActiveForm::begin(['options' => ['enctype' => 'multipart/form-data']]) ?>
                    <img src="<?php echo Url::to('@web/uploads/'.$text->img);?>" style="width:100%">
                    <?= $form->field($model, 'imageFile')->fileInput()->label('产品图片'); ?>
                    图片比例为388：259
                    <img id="cropedBigImg" value='custom' alt="lorem ipsum dolor sit" data-address='' title="裁剪" style="display:none;"/>
                    
                    <?= $form->field($text, 'title')->label('标题'); ?>
                    <?= $form->field($text, 'date')->label('日期')->widget(DateTimePicker::classname(), [
                        'options' => ['value' => date('Y-m-d',$text->date)],
                        'removeButton' => false,
                        'pluginOptions' => [
                            'autoclose'=>true,
                    ...
                    ?>
                    <?= $form->field($text, 'js')->label('简要内容'); ?>
                    <?= $form->field($text, 'content')->label('内容')->widget('yii\ueditor\Ueditor'); ?>
                    <?= Html::activeHiddenInput($text,'type',array('value'=>'2')) ?>
