...
<section class="row product-details">
	<div class="col-md-1 no-padding">
		<img id="detail-guide" width="100%" src="<?php echo base_url();?>assets/images/detail-guide.png" style="position:absolute;left:0px;">
	</div>
	<div class="col-md-10 no-padding details">
		<div class="col-md-12 no-padding product-title-padding" style="position:relative;background-color:#fff;">
			<div class="col-md-3 product-title">
				<h3 style="font-family:'微软雅黑';font-style:normal"><?php echo $cpzx->cpmc;?></h3>
			</div>
			<div class="col-md-5 no-padding product-color">
				<div class="col-md-3 no-padding psfa" style="text-align:center;"><h3>配色方案</h3></div>
				<div class="col-md-7 no-padding color-select"></div>
			</div>
		</div>
		<div class="col-md-12 no-padding" style="position:relative;background-color:#f5f5f5;">
			<div class="col-md-7 no-padding image-view">
				<img id="image-v" src="<?php echo uploads().$cpzx->img;?>" width="100%">
			</div>
			<div class="col-md-1 no-padding images-scroll" style="position:relative;margin-top:5%;">
				<div class="product-scroll">
				</div>
			</div>
			<div class="col-md-4 no-padding images-list">
			<?php foreach($cpps as $row){?>
			...
			<div class="row">
				<div class="col-md-6">
					...
					<div class="col-md-8 kxcc">
						<select onchange="changejg(this.value)">
							<?php
							$i=1;
							$jg=0; 
							foreach($cpxx as $row){
								...
								$i++;
								?>
								<option value ="<?php echo $row->cpjg;?>"><?php echo $row->cpcc?>"</option>
							<?php }?>
						</select>
					</div>
				</div>
				<div class="col-md-6">
					...
					<div class="col-md-6 price">
						<h3 id="cpjg_change"><?php echo $jg;?></h3>
					</div>
				</div>
				<script>
				function changejg(value){
					document.getElementById("cpjg_change").innerHTML=value;
				}
				</script>
			</div>
		</div>
		<!--div class="col-md-12 no-padding details-bg2">
			<h3 style="margin-top:3%;">产品描述</h3>
			<h3>Description</h3>
		</div-->
		<div class="col-md-12 no-padding details-bg2" <?php if($cpzx->cpmsimg){?> style="background-image:url(<?php echo uploads().$cpzx->cpmsimg;?>);"<?php }?>>
			<h3 style="margin-top:3%;">产品描述</h3>
			<h3>Description</h3>
		</div>
		<?php $cpms_arr=unserialize($cpzx->cpms);
		
		foreach($cpms_arr as $key=>$value){
			
		?>
		<div class="col-md-12 no-padding des-a" >
			<img src="<?php echo uploads().$value?>" width="100%">
		</div>
		<?php }?>
	</div>
	<div class="col-md-1 no-padding" >
...
$(document).ready(function () { 
	...
	$(".images-list").css('height',$("#image-v").height()*0.9);
	$(".images-scroll").css('height',$("#image-v").height());
	$(".product-scroll").css('height',$(".images-list").height());
	$(".product-title-padding").css('height',$("#detail-guide").height()/5);
	$(".color-select").css('height',$(".psfa").height());
    $(".color-select").animate({width:'+=210px'},500);
	$(".images-list img").mousedown(function(){
		$('#image-v').attr('src',$(this)[0].src);
	});
});
</script> 
